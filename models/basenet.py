...
from models.utils import standard_val, standard_test, gdro_val
from datasets.utils import get_dataset
from utils.evaluation import calculate_metrics, calculate_FPR_FNR, find_threshold
...
class BaseNet(nn.Module):
    ...
    def __init__(self, opt, wandb):
        ...
        self.gdro_val_loss_for_baseline = opt['gdro_val_loss_for_baseline'] # options: 'None', 'Standard', 'Pure'
    ...
    def _val(self, loader):
        ...
        self.network.eval()
        if self.gdro_val_loss_for_baseline == 'Pure':
            print('doing pure gdro val in baseline')
            auc, val_loss, log_dict, pred_df = gdro_val(self.opt, self.network, loader, nn.BCEWithLogitsLoss(reduction = 'none'), self.sens_classes, self.wandb, self.val_data, pure_loss=True)
        elif self.gdro_val_loss_for_baseline == 'Standard':
            print('doing standard gdro val in baseline')
            auc, val_loss, log_dict, pred_df = gdro_val(self.opt, self.network, loader, nn.BCEWithLogitsLoss(reduction = 'none'), self.sens_classes, self.wandb, self.val_data, pure_loss=False)
        else:
    ...
    def _test(self, loader):
        ...
        if self.pretrained:
            if self.cross_testing:
                pred_df.to_csv(os.path.join(self.save_path, 'pretrained_ext_pred.csv'), index = False)
            else:
    ...
    def record_val(self):
        ...
        self.best_pred_df.to_csv(os.path.join(self.save_path, 'best_val_pred.csv'), index = False)
