...
class UKBB_RET_REG(BaseDataset):
    def __init__(self, dataframe, path_to_pickles, sens_name, sens_classes, transform,use_pkl=True):
        super(UKBB_RET_REG, self).__init__(dataframe, path_to_pickles, sens_name, sens_classes, transform)
        ...
        self.Y = (np.asarray(self.dataframe['rescaled_4080'].values)).astype('float')
    ...
    def __getitem__(self, idx):
        ...
        label = torch.FloatTensor([float(item['rescaled_4080'])])
