...
class OrthoLoss(nn.Module):
    ...
    def forward(self, inputs, target, sensitive, current_step):
        ...
        for i in range(z1.shape[0]):
            ...
            if not torch.all(torch.isfinite(log_std_t)):
                print(f"Invalid log_std_t values: {log_std_t}")
            device = mean_t[i].device  # Get the device of `mean_t[i]`
            n_t = MultivariateNormal(
                mean_t[i],
                torch.diag(torch.exp(log_std_t[i])) + torch.eye(128, device=device) * 1e-6
            )   # added 1e-6 to avoid loss of positive definiteness
            n_s = MultivariateNormal(
                mean_s[i],
                torch.diag(torch.exp(log_std_t[i])) + torch.eye(128, device=device) * 1e-6
            )
