...
class LossComputer:
    def __init__(self, criterion, is_robust, dataset, alpha=None, gamma=0.1, adj=None, min_var_weight=0, step_size=0.01, normalize_loss=False, btl=False,pure=False):
...
=        group_acc, group_count = self.compute_group_avg((yhat > 0.5).float(), group_idx)
        ...
        if self.pure:
            actual_loss, index = self.compute_pure_robust_loss(group_loss, group_count)
            _, weights = self.compute_robust_loss(group_loss, group_count) # not efficient but just to get weights
        elif self.is_robust and not self.btl:
            actual_loss, weights = self.compute_robust_loss(group_loss, group_count)
            index = None
        elif self.is_robust and self.btl:
             actual_loss, weights, index = self.compute_robust_loss_btl(group_loss, group_count)
        ...
        return group_loss,per_sample_losses.mean(),actual_loss,index,weights

    def compute_robust_loss(self, group_loss, group_count):
        adjusted_loss = group_loss
        # add adjustment term (proportional to group size so that small groups get higher loss) - none done if adj = 0
        if torch.all(self.adj>0):
            adjusted_loss += self.adj/torch.sqrt(self.group_counts)
        if self.normalize_loss:
            adjusted_loss = adjusted_loss/(adjusted_loss.sum())
        
        # adv probs initiliased as 1/(n_groups), and adjusted based on loss for each group, wiht step size controlling size of increase (higher loss => higher adv probs)
        self.adv_probs = self.adv_probs * torch.exp(self.step_size*adjusted_loss.data)
        # re-normalise adv probs so it sums to 1
        self.adv_probs = self.adv_probs/(self.adv_probs.sum())

        # multiply each group loss by its adv_probs and sum so that groups with higher losses will have bigger weighting (weighting is also carried over from previous training history)
    ...
    def compute_robust_loss_greedy(self, group_loss, ref_loss):
        sorted_idx = ref_loss.sort(descending=True)[1] # ref loss isn't loss at that group its the exp average loss so far with size adjustment
        _, index = ref_loss.max(0)
        sorted_loss = group_loss[sorted_idx]

        # group frac is proportion of people in each group, sort group proportions in order of descending loss
        sorted_frac = self.group_frac[sorted_idx]
	
        mask = torch.cumsum(sorted_frac, dim=0)<=self.alpha
        weights = mask.float() * sorted_frac /self.alpha
        last_idx = mask.sum() # counts number of true values
        weights[last_idx] = 1 - weights.sum() # this is just to check weights sum to 1, if not gets next weight (eg at index position 2 in my example)
    
        weights = sorted_frac*self.min_var_weight + weights*(1-self.min_var_weight)# min var weight is 0 so weights shouldn't change
        ...
        return robust_loss, unsorted_weights, index
    
    def compute_pure_robust_loss(self, group_loss, group_count):
        max_loss, index = group_loss.max(0)
        return max_loss, index

    def compute_group_avg(self, losses, group_idx):
        # compute observed counts and mean loss for each group
        group_map = (group_idx == torch.arange(self.n_groups).unsqueeze(1).long().cuda()).float() #size: n_groups x batch_size (think this is a tensor saying whether or not the sample is in the group)
    ...
    def update_exp_avg_loss(self, group_loss, group_count):
        prev_weights = (1 - self.gamma*(group_count>0).float()) * (self.exp_avg_initialized>0).float() # gamma just weights how much to include current loss in exp avg loss
    ...
    def update_stats(self, actual_loss, group_loss, group_acc, group_count, weights=None):
        ...
        # this is similar to exp_avg_loss just calculated slightly differently, just based on size of groups (doesn't decay over time i guess)
