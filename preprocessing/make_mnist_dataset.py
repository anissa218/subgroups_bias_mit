...
if __name__ == "__main__":
    ...
    AY_map = {
        (0, 0): 0,  # A=0, binaryLabel=0 -> AY=0
        (0, 1): 1,  # A=0, binaryLabel=1 -> AY=1
        (1, 0): 2   # A=1, binaryLabel=0 -> AY=2
    }
    ...
    subset_train_val_images_df['S'] = subset_train_val_images_df['Sex'].apply(lambda x: 0 if x == 'M' else 1)
    subset_train_val_images_df['A'] = subset_train_val_images_df.apply(assign_A, axis=1,prop_spurious_male = prop_spurious_images_male_train_val,prop_spurious_female=prop_spurious_images_female_train_val)
    subset_train_val_images_df['AY'] = subset_train_val_images_df.apply(
        lambda row: AY_map.get((row['A'], row['binaryLabel']), default_AY), axis=1
    )
    ...
    subset_test_images_df['S'] = subset_test_images_df['Sex'].apply(lambda x: 0 if x == 'M' else 1)
    subset_test_images_df['A'] = subset_test_images_df.apply(assign_A, axis=1,prop_spurious_male = prop_spurious_images_male_test,prop_spurious_female=prop_spurious_images_female_test)
    ...
    for df in dfs:
        ...
        df['YAS'] = df[['Y','A','S']].apply(tuple, axis=1).map(YAS_map)
        df['AS'] = df[['A','S']].apply(tuple, axis=1).map(AS_map)
        df['A_4'] = [x if random.random() < 0.5 else x + 2 for x in df['A']]
        df['S_4'] = [x if random.random() < 0.5 else x + 2 for x in df['S']]

        for error_percent in [0.01, 0.05, 0.1, 0.25, 0.5]:
            error_col = f'noisy_A_{int(error_percent * 100):03}'
            df[error_col] = add_noise(df, 'A', error_percent)['A']
        
        for error_percent in [0.01, 0.05, 0.1, 0.25, 0.5]:
            error_col = f'noisy_S_{int(error_percent * 100):03}'
            df[error_col] = add_noise(df, 'S', error_percent)['S']
    ...
    # MAKE IMAGES
    subset_train_images = make_subset_images(subset_train_images_df,train_images,fg_colour_channel_col='S',bg_colour_channel_col = 'A',noise=1)
    subset_val_images = make_subset_images(subset_val_images_df,train_images,fg_colour_channel_col='S',bg_colour_channel_col = 'A',noise=1)
    subset_test_images = make_subset_images(subset_test_images_df,test_images,fg_colour_channel_col='S',bg_colour_channel_col = 'A',noise=1)
