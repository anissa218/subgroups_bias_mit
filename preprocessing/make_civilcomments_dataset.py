...
from utils.attr_definitions import GROUP_ATTRS, AGGREGATE_ATTRS, ORIG_ATTRS

def parse_args():
    parser = argparse.ArgumentParser(description="Generate CelebA dataset with bias.")
    parser.add_argument("--raw_data_folder", type=str, required=True, help="Path to the raw Civil_comments data folder ")
...
if __name__ == "__main__":
    ...
    annot_data_df = load_civilcomments_df(os.path.join(raw_data_folder))
    processed_annot_data_df = augment_civilcomments_df(annot_data_df)
    processed_annot_data_df['binary_label'] = (processed_annot_data_df['toxicity'] >= 0.5).astype(int)
    subset_df = processed_annot_data_df[['id','comment_text','toxicity','gender_any','orientation_any','religion_any','race_any','disability_any','binary_label']]
    subset_df['A'] = subset_df['gender_any']
    subset_df['S'] = subset_df['religion_any']
    subset_df.drop(columns=['gender_any','religion_any'],inplace=True)
    ...
    samples_per_group = {
        ...
        '111': int(total_images * prop_111),  # All available images
