...
def get_dataset(opt):
    ...
    if opt['is_3d']:
        ...
        if data_setting['augment']:
            if opt['dataset_name'] == 'CXP':
                print('doing cxp specific augmentation')
                transform_train = transforms.Compose([
    ...
    if opt['dataset_name'] == 'UKBB_RET' or opt['dataset_name'] == 'MNIST' or opt['dataset_name'] == 'CXP': # account for having differnet csvs (different binary labels) depending on what prediction task you are doing
        ...
        if opt['cross_testing']:
            test_meta = pd.read_csv(os.path.join(root_path,data_folder,data_setting['ext_test' + class_name+ '_meta_path']))
            print('loaded special ext test dataset')
        ...
        if opt['cross_testing']:
            test_meta = pd.read_csv(data_setting['ext_test_meta_path'])
            print('loaded special ext test dataset')
        else:
            test_meta = pd.read_csv(data_setting['test_meta_path'])   

    ### LOAD IMAGES / INSTANTIATE DATASET CLASS ###    
    ...
    if opt['is_3d']:
        ...
        if  opt['dataset_name'] == 'UKBB_RET' or opt['dataset_name'] == 'MNIST' or opt['dataset_name'] == 'CXP': # get specific images for the desired data_folder
            ...
            if opt['cross_testing']:
                pickle_test_path = os.path.join(root_path,data_folder,data_setting['pickle_ext_test_path'])
                print('loaded special ext test dataset')
            ...
            if opt['cross_testing']:
                pickle_test_path = data_setting['pickle_ext_test_path']
                print('loaded special ext test pickle images')
            else:
