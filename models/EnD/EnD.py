...
from models.EnD.model import EnDNet, EnDNet3D,EnDNetMLP, EnDNetSimpleCNN
...
class EnD(BaseNet):
    ...
    def set_network(self, opt):
        ...
        if self.is_3d:
            ...
            self.network = EnDNetSimpleCNN(backbone = self.backbone, n_classes = self.num_classes).to(self.device)
        else:
        ...
        for target_class in target_classes:
            ...
            for idx, bias_class in enumerate(bias_classes):
                ...
                for other_bias_class in bias_classes[idx:]:
                    ...
                    mask = torch.tril(
                            torch.mm(
                                (class_mask * bias_mask).squeeze(-1),
                                (class_mask * other_bias_mask).squeeze(-1).T
                            ),
                            diagonal=-1
                        )
                    # mask = torch.tril(
